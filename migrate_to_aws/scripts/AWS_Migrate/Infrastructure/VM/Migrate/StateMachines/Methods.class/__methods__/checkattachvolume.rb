#
# Description: Method: CheckAttachVolume
#

require 'net/ssh'

begin
  @method = 'CheckAttachVolume'
  ...
  while disk < vm.num_hard_disks + 1 do
    ...
    Net::SSH.start(migrate_host, migrate_user) do |ssh|
      output = ssh.exec!("source .bash_profile; ec2-describe-volumes -O #{aws.authentication_userid} -W #{aws.authentication_password} --region #{aws.provider_region} #{volumeid}")

      $evm.log('info', "#{@method} - Inspect output: #{output.inspect}") if @debug

      unless output =~ /attached/
